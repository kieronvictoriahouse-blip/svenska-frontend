...
var CATS=[{id:'Épices',emoji:'🌶️'},{id:'Flocons & Céréales',emoji:'🌾'},{id:'Baies séchées',emoji:'🫐'},{id:'Sucres & Sirops',emoji:'🍯'},{id:'Thés & Tisanes',emoji:'🫖'},{id:'Farines & Graines',emoji:'🌻'},{id:'Mélanges',emoji:'🎄'},{id:'Chips & Snacks',emoji:'🍟'},{id:'Basics suédois',emoji:'🇸🇪'},{id:'Confiseries',emoji:'🍬'},{id:'Fika & Boulangerie',emoji:'🧁'},{id:'Art de la table',emoji:'🍽️'},{id:'Maison & Déco',emoji:'🕯️'}];
...
const UI={sv:{addedCart:'Tillagd i korgen!',emptyCart:'Din korg är tom',subtotal:'Delsumma',checkout:'Beställ →',keepShopping:'Fortsätt handla',freeShip:'Fri frakt från 29€',searchPlaceholder:'Sök en krydda, ett produkt...',noResults:'Inga resultat',cartTitle:'Din korg',products:'produkter',product:'produkt',filterAll:'Alla',sortDefault:'Standard',sortPriceAsc:'Pris stigande',sortPriceDesc:'Pris fallande',sortName:'Namn A→Ö',subscribe:'Tack!'},fr:{addedCart:'Ajouté au panier !',emptyCart:'Votre panier est vide',subtotal:'Sous-total',checkout:'Commander →',keepShopping:'Continuer mes achats',freeShip:'Livraison offerte dès 29€',searchPlaceholder:'Rechercher une épice, un produit...',noResults:'Aucun résultat',cartTitle:'Votre panier',products:'produits',product:'produit',filterAll:'Tous',sortDefault:'Par défaut',sortPriceAsc:'Prix croissant',sortPriceDesc:'Prix décroissant',sortName:'Nom A→Z',subscribe:'Merci !'},en:{addedCart:'Added to cart!',emptyCart:'Your cart is empty',subtotal:'Subtotal',checkout:'Checkout →',keepShopping:'Continue shopping',freeShip:'Free delivery from €29',searchPlaceholder:'Search for a spice, product...',noResults:'No results',cartTitle:'Your cart',products:'products',product:'product',filterAll:'All',sortDefault:'Default',sortPriceAsc:'Price: low to high',sortPriceDesc:'Price: high to low',sortName:'Name A→Z',subscribe:'Thanks!'}};
...
window.SD_SHIP=(function(){
  var DEF={FR:{threshold:29,cost:4.90},INTL:{threshold:50,cost:9.90}};
...
function _doSekRefresh(){renderCartDrawer();if(typeof renderCart==='function')renderCart();document.querySelectorAll('.prod-price,.prod-price-old').forEach(function(el){var m=el.textContent.match(/€([\d.]+)/);if(m)el.textContent=Math.round(parseFloat(m[1])*_sekRate)+' kr';});var pp=document.getElementById('pdp-price');if(pp){var m=pp.textContent.match(/€([\d.]+)/);if(m)pp.textContent=Math.round(parseFloat(m[1])*_sekRate)+' kr';}var ppo=document.getElementById('pdp-price-old');if(ppo){var mo=ppo.textContent.match(/€([\d.]+)/);if(mo)ppo.textContent=Math.round(parseFloat(mo[1])*_sekRate)+' kr';}var spo=document.getElementById('sticky-atc-price-old');if(spo){var ms=spo.textContent.match(/€([\d.]+)/);if(ms)spo.textContent=Math.round(parseFloat(ms[1])*_sekRate)+' kr';}var sp=document.getElementById('sticky-atc-price');if(sp){var m2=sp.textContent.match(/€([\d.]+)/);if(m2)sp.textContent=Math.round(parseFloat(m2[1])*_sekRate)+' kr';}}
...
// ─── Tracking publicitaire : Meta Pixel + e-commerce GA4 (pour lancer des pubs mesurées) ───
// POUR ACTIVER META : coller l'ID du Pixel Meta ci-dessous (ex. '1234567890123456'). Vide = inactif.
window.SD_META_PIXEL_ID = window.SD_META_PIXEL_ID || '';
(function(){
  if(!window.SD_META_PIXEL_ID || window.fbq) return;
  !function(f,b,e,v,n,t,s){if(f.fbq)return;n=f.fbq=function(){n.callMethod?n.callMethod.apply(n,arguments):n.queue.push(arguments)};if(!f._fbq)f._fbq=n;n.push=n;n.loaded=!0;n.version='2.0';n.queue=[];t=b.createElement(e);t.async=!0;t.src=v;s=b.getElementsByTagName(e)[0];s.parentNode.insertBefore(t,s)}(window,document,'script','https://connect.facebook.net/en_US/fbevents.js');
  fbq('init', window.SD_META_PIXEL_ID); fbq('track','PageView');
})();
/* Événements e-commerce GA4 (importés dans Google Ads). item_id = UUID produit = g:id du flux Merchant : c'est ce qui relie une conversion au produit Shopping cliqué et alimente le remarketing dynamique. Ne pas changer l'un sans l'autre. */
function _sdGaPrice(p){var b=(p&&p.price)||0;return window.SDPrice?SDPrice.effective(b,p):b;}
function _sdGaItem(p,qty){return {item_id:String((p&&(p.uuid||p.id))||''),item_name:(p&&p.name&&(p.name.fr||p.name))||'',item_category:(p&&p.cat)||'',quantity:qty||1,price:_sdGaPrice(p)};}
function sdTrackViewItem(p){try{if(!p||typeof gtag!=='function')return;var id=String(p.uuid||p.id);if(window._sdViewed===id)return;window._sdViewed=id;var it=_sdGaItem(p,1);gtag('event','view_item',{currency:'EUR',value:it.price,items:[it]});}catch(e){}}
function sdTrackAddToCart(p,qty){try{var it=_sdGaItem(p,qty);var v=Math.round(it.price*it.quantity*100)/100;if(typeof gtag==='function')gtag('event','add_to_cart',{currency:'EUR',value:v,items:[it]});if(window.fbq)fbq('track','AddToCart',{value:v,currency:'EUR'});}catch(e){}}
function sdTrackBeginCheckout(){try{if(typeof gtag!=='function')return;var cp={};try{cp=JSON.parse(localStorage.getItem('sd_cart_products')||'{}');}catch(e){}var items=[],v=0;Object.entries(cart).filter(function(e){return e[1]>0;}).forEach(function(e){var k=e[0],i=k.indexOf('_'),id=i===-1?k:k.slice(0,i);var p=PRODUCTS.find(function(x){return String(x.uuid||x.id)===id;})||cp[id];if(!p)return;var it=_sdGaItem(Object.assign({uuid:id},p),e[1]);it.item_id=id;items.push(it);v+=it.price*it.quantity;});if(items.length)gtag('event','begin_checkout',{currency:'EUR',value:Math.round(v*100)/100,items:items});}catch(e){}}
function sdTrackPurchase(order){try{if(!order)return;var total=parseFloat(order.total)||0;var tid=String(order.order_number||order.id||'');var L=[];try{L=typeof order.lines==='string'?JSON.parse(order.lines):(order.lines||[]);}catch(e){}var items=L.map(function(l){return {item_id:String(l.product_id||''),item_name:l.name||'',quantity:l.qty||1,price:parseFloat(l.price)||0};});if(typeof gtag==='function'){gtag('event','purchase',{transaction_id:tid,value:total,shipping:parseFloat(order.shipping)||0,currency:'EUR',items:items});var A=window.SD_ADS;if(A&&A.id&&A.purchaseLabel)gtag('event','conversion',{send_to:A.id+'/'+A.purchaseLabel,value:total,currency:'EUR',transaction_id:tid});}if(window.fbq)fbq('track','Purchase',{value:total,currency:'EUR'});}catch(e){}}

// ── Stock : helpers partagés (panier + page produit) ─────────────────────────
...
function _snipAdd(){}
function changeQty(key,delta){var ri=key.indexOf('_');var cid=ri===-1?key:key.slice(0,ri);var p=_sdFindProd(cid);var stock=_sdStock(p);var target=(cart[key]||0)+delta;if(target>0&&stock!==Infinity){var others=_sdCartQtyForProduct(cid)-(cart[key]||0);var maxThis=stock-others;if(target>maxThis){target=Math.max(0,maxThis);showToast(_sdStockMsg('only',stock));}}if(target<=0)delete cart[key];else cart[key]=target;saveCart();updateCartBadge();renderCartDrawer();}
...
/* ── NEWSLETTER (capture email → CRM contacts, exploitable par le module Marketing) ── */
function sdNewsletterSubscribe(email){
  var base=(typeof SD_API_URL!=='undefined'&&SD_API_URL)?SD_API_URL:'https://admin.swedishcravings.fr';
  return fetch(base+'/api/newsletter',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:email})})
    .then(function(r){return r.ok;}).catch(function(){return false;});
}
function sdFooterNlSubmit(){
  var el=document.getElementById('footer-nl-email');if(!el)return;
  var email=(el.value||'').trim();var msg=document.getElementById('footer-nl-msg');
  if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)){if(msg)msg.textContent=(LANG==='sv'?'Ogiltig e-post':LANG==='en'?'Invalid email':'E-mail invalide');return;}
  if(msg)msg.textContent='…';
  sdNewsletterSubscribe(email).then(function(ok){
    if(msg)msg.textContent=ok?(LANG==='sv'?'Tack! 🎉':LANG==='en'?'Thanks! 🎉':'Merci, c\'est noté ! 🎉'):(LANG==='sv'?'Försök igen':LANG==='en'?'Please try again':'Réessayez');
    if(ok)el.value='';
  });
}

function productCardHTML(p){const badge=p.badge?`<div class="prod-badge ${p.badge}">${getBadgeLabel(p.badge)}</div>`:'';const _onSale=!!(window.SDPrice&&SDPrice.active(p));const _salePrice=_onSale?SDPrice.effective(p.price,p):(p.price||0);const promoBadge=_onSale?`<div class="prod-badge badge-promo">${LANG==='en'?'Special offer':LANG==='sv'?'Erbjudande':'Offre spéciale'}</div>`:'';const priceHtml=_onSale?`<span class="prod-price-old">${fmtPrice(p.price||0)}</span><span class="prod-price prod-price--sale">${fmtPrice(_salePrice)}</span>`:`<span class="prod-price">${fmtPrice(p.price||0)}</span>`;const rating=parseFloat(p.rating)||4.5;const origin=typeof p.origin==='object'?p.origin[LANG]||'':p.origin||'';const navId=p.uuid||p.id;const addId=p.uuid?`'${p.uuid}'`:(typeof p.id==='string'?`'${p.id}'`:p.id);const isFav=_favorites.includes(String(navId));const _rawW=p.weight||'';const _cleanW=_rawW.replace(/^\d+\s*[xX×]\s*/,'').trim();const weightHtml=_cleanW?`<span class="prod-weight">${_cleanW}</span>`:'';let stockBadge='';const _isOut=(p.trackStock&&p.stock!==null&&p.stock!==undefined&&p.stock<=0);if(p.trackStock&&p.stock!==null){if(p.stock<=0){const l={fr:'Coming soon',en:'Coming soon',sv:'Coming soon'};stockBadge=`<div class="prod-stock-badge psb-out">${l[LANG]||l.fr}</div>`;}else if(p.stock<=2){const l={fr:'Stock limité',en:'Limited stock',sv:'Begränsat'};stockBadge=`<div class="prod-stock-badge psb-low">${l[LANG]||l.fr}</div>`;}else{const l={fr:'En stock',en:'In stock',sv:'I lager'};stockBadge=`<div class="prod-stock-badge psb-ok">${l[LANG]||l.fr}</div>`;}}return`<div class="prod-card" onclick="window.location.href='produit.html?id=${navId}'"><div class="prod-img">${badge}${promoBadge}${stockBadge}<div class="prod-img-inner"><img src="${p.photo||''}" alt="${p.name?.['fr']||''}" loading="lazy" onerror="this.style.display='none'" style="width:100%;height:100%;object-fit:contain;padding:8px;background:var(--cream);"></div><button class="prod-quick"${_isOut?' disabled style="opacity:.35;cursor:not-allowed" onclick="event.stopPropagation()"':` onclick="event.stopPropagation();addToCart(${addId})"`} title="Ajouter"><svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.8"><line x1="7" y1="1" x2="7" y2="13"/><line x1="1" y1="7" x2="13" y2="7"/></svg></button><button class="prod-wish${isFav?' liked':''}" data-fav-id="${navId}" onclick="event.stopPropagation();toggleFavorite('${navId}')"><svg width="14" height="13" viewBox="0 0 14 13" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M7 11.5C7 11.5 1 7.8 1 4a3 3 0 0 1 6 0 3 3 0 0 1 6 0c0 3.8-6 7.5-6 7.5z"/></svg></button></div><div class="prod-body"><div class="prod-cat">${CAT_LABELS[p.cat]?.[LANG]||p.cat||''}</div><div class="prod-name">${p.name?.[LANG]||''}</div><div class="prod-origin">${origin}</div><div class="prod-footer">${priceHtml}${weightHtml}</div></div></div>`;}
...
function renderHeader(activePage=''){const _searchIcon='<svg width="18" height="18" viewBox="0 0 18 18" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"><circle cx="7.5" cy="7.5" r="5.5"/><line x1="11.5" y1="11.5" x2="17" y2="17"/></svg>';const _closeIcon='<svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"><line x1="1" y1="1" x2="13" y2="13"/><line x1="13" y1="1" x2="1" y2="13"/></svg>';return`<div class="lang-bar"><button class="lang-btn${LANG==='sv'?' active':''}" data-lang="sv" onclick="setLang('sv')">Svenska</button><button class="lang-btn${LANG==='fr'?' active':''}" data-lang="fr" onclick="setLang('fr')">Français</button><button class="lang-btn${LANG==='en'?' active':''}" data-lang="en" onclick="setLang('en')">English</button></div><div class="announcement" data-sv="${SD_WL.announcement.sv}" data-fr="${SD_WL.announcement.fr}" data-en="${SD_WL.announcement.en}">${SD_WL.announcement[LANG]}</div>${sdSeasonBannerHTML()}<header><div class="nav-inner"><a href="index.html" class="logo"><img class="logo-mark" src="/img/sc-monogramme.png" alt="${SD_WL.brand_name}" width="28" height="42"><span class="logo-text"><span class="logo-main">${SD_WL.brand_name}</span><span class="logo-tag">${SD_WL.brand_tagline}</span></span></a><nav><a href="index.html"${activePage==='home'?' class="active"':''} data-sv="Hem" data-fr="Accueil" data-en="Home">Accueil</a><a href="boutique.html"${activePage==='shop'?' class="active"':''} data-sv="Butiken" data-fr="Boutique" data-en="Shop">Boutique</a><a href="maison.html"${activePage==='maison'?' class="active"':''} data-sv="Home" data-fr="Home" data-en="Home">Home</a><a href="recettes.html"${activePage==='recipes'?' class="active"':''} data-sv="Recept" data-fr="Recettes" data-en="Recipes">Recettes</a><a href="a-propos.html"${activePage==='about'?' class="active"':''} data-sv="Vår historia" data-fr="Notre histoire" data-en="Our story">Notre histoire</a><a href="contact.html"${activePage==='contact'?' class="active"':''} data-sv="Contact" data-fr="Contact" data-en="Contact">Contact</a>${(window.SD_PAGES||[]).filter(p=>p.show_in_nav).map(p=>`<a href="page.html?slug=${p.slug}"${activePage===p.slug?' class="active"':''} data-sv="${p.nav_label_sv||p.title_sv}" data-fr="${p.nav_label_fr||p.title_fr}" data-en="${p.nav_label_en||p.title_en}">${p.nav_label_fr||p.title_fr}</a>`).join('')}</nav><div class="nav-actions"><button class="btn-search" onclick="openSearch()" aria-label="Rechercher">${_searchIcon}</button><button class="btn-cart" onclick="openCart()" aria-label="Panier"><svg class="cart-ico" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"/><path d="M3 6h18"/><path d="M16 10a4 4 0 0 1-8 0"/></svg><span class="cart-label" data-sv="Korg" data-fr="Panier" data-en="Cart">Panier</span><span class="cart-badge">${Object.values(cart).reduce((a,b)=>a+b,0)}</span></button><button class="hamburger" onclick="openMob()"><span></span><span></span><span></span></button></div></div></header><div class="mobile-nav" id="mobile-nav"><button class="mob-close" onclick="closeMob()" aria-label="Fermer">${_closeIcon}</button><a href="index.html" onclick="closeMob()" data-sv="Hem" data-fr="Accueil" data-en="Home">Accueil</a><a href="boutique.html" onclick="closeMob()" data-sv="Butiken" data-fr="Boutique" data-en="Shop">Boutique</a><a href="maison.html" onclick="closeMob()" data-sv="Home" data-fr="Home" data-en="Home">Home</a><a href="recettes.html" onclick="closeMob()" data-sv="Recept" data-fr="Recettes" data-en="Recipes">Recettes</a><a href="a-propos.html" onclick="closeMob()" data-sv="Vår historia" data-fr="Notre histoire" data-en="Our story">Notre histoire</a><a href="contact.html" onclick="closeMob()" data-sv="Contact" data-fr="Contact" data-en="Contact">Contact</a>${(window.SD_PAGES||[]).filter(p=>p.show_in_nav).map(p=>`<a href="page.html?slug=${p.slug}" onclick="closeMob()" data-sv="${p.nav_label_sv||p.title_sv}" data-fr="${p.nav_label_fr||p.title_fr}" data-en="${p.nav_label_en||p.title_en}">${p.nav_label_fr||p.title_fr}</a>`).join('')}</div><div class="search-overlay" id="search-overlay" onclick="handleSearchClick(event)"><div class="search-box"><div class="search-row"><span class="search-icon-wrap">${_searchIcon}</span><input type="text" id="search-inp" oninput="doSearch(this.value)" data-sv-ph="Sök..." data-fr-ph="Rechercher..." data-en-ph="Search..." placeholder="Rechercher..."><button class="search-close" onclick="closeSearch()" aria-label="Fermer">${_closeIcon}</button></div><div class="search-results" id="search-res"></div></div></div><div class="drawer-overlay" id="drawer-overlay" onclick="closeCart()"></div><div class="cart-drawer" id="cart-drawer"><div class="drawer-head"><h3 data-sv="Din korg" data-fr="Votre panier" data-en="Your cart">Votre panier</h3><button class="drawer-close" onclick="closeCart()" aria-label="Fermer">${_closeIcon}</button></div><div class="drawer-items" id="drawer-items-list"></div><div class="drawer-foot" id="drawer-footer" style="display:none;"><div class="cart-subtotal-row"><span data-sv="Delsumma" data-fr="Sous-total" data-en="Subtotal">Sous-total</span><span id="cart-total-price">€0.00</span></div><p class="cart-free-ship" id="cart-ship-note"></p><button class="btn-checkout" onclick="openSnipcartCheckout()" data-sv="Beställ →" data-fr="Commander →" data-en="Checkout →">Commander →</button><button class="btn-keep-shopping" onclick="closeCart()" data-sv="Fortsätt handla" data-fr="Continuer mes achats" data-en="Continue shopping">Continuer mes achats</button></div></div><div class="toast" id="toast"></div>`;}

function renderFooter(){return`<footer><div class="footer-inner"><div class="footer-grid"><div class="footer-logo"><a href="index.html" class="logo"><span class="logo-main">${SD_WL.brand_name}</span><span class="logo-tag">${SD_WL.brand_tagline}</span></a><p data-sv="${SD_WL.footer_desc.sv}" data-fr="${SD_WL.footer_desc.fr}" data-en="${SD_WL.footer_desc.en}">${SD_WL.footer_desc[LANG]}</p><div class="footer-social">${SD_WL.instagram?`<a href="${SD_WL.instagram}" class="soc-link" target="_blank" rel="noopener" aria-label="Instagram">IG</a>`:''}${SD_WL.facebook?`<a href="${SD_WL.facebook}" class="soc-link" target="_blank" rel="noopener" aria-label="Facebook">FB</a>`:''}${SD_WL.pinterest?`<a href="${SD_WL.pinterest}" class="soc-link" target="_blank" rel="noopener" aria-label="Pinterest">PIN</a>`:''}</div><div class="footer-nl" style="margin-top:22px;max-width:280px;"><div style="font-family:var(--font-ui);font-size:10px;letter-spacing:2px;text-transform:uppercase;color:rgba(255,255,255,0.5);margin-bottom:8px;" data-sv="Nyhetsbrev — nyheter & erbjudanden" data-fr="Newsletter — nouveautés & offres" data-en="Newsletter — news & offers">Newsletter — nouveautés & offres</div><div style="display:flex;gap:6px;"><input id="footer-nl-email" type="email" data-sv-ph="Din e-post" data-fr-ph="Votre e-mail" data-en-ph="Your email" placeholder="Votre e-mail" style="flex:1;min-width:0;background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);border-radius:2px;padding:9px 10px;font-family:var(--font-body);font-size:13px;color:#fff;outline:none;"><button onclick="sdFooterNlSubmit()" style="background:var(--lingon);color:#fff;border:none;border-radius:2px;padding:9px 14px;font-family:var(--font-ui);font-size:11px;letter-spacing:1px;cursor:pointer;">OK</button></div><div id="footer-nl-msg" style="font-family:var(--font-body);font-size:12px;color:rgba(255,255,255,0.55);margin-top:6px;min-height:16px;"></div></div></div><div class="footer-col"><h4 data-sv="Butiken" data-fr="La boutique" data-en="Shop">La boutique</h4><ul><li><a href="boutique.html?cat=%C3%89pices" data-sv="Kryddor & Örter" data-fr="Épices & Aromates" data-en="Spices & Herbs">Épices & Aromates</a></li><li><a href="boutique.html?cat=Chips+%26+Snacks">Chips & Snacks</a></li><li><a href="boutique.html?cat=Confiseries" data-sv="Konfektyr" data-fr="Confiseries" data-en="Confectionery">Confiseries</a></li><li><a href="boutique.html?cat=Basics+su%C3%A9dois" data-sv="Svenska baser" data-fr="Basics suédois" data-en="Swedish basics">Basics suédois</a></li><li><a href="boutique.html?cat=Fika+%26+Boulangerie" data-sv="Fika & Bakning" data-fr="Fika & Boulangerie" data-en="Fika & Baking">Fika & Boulangerie</a></li><li><a href="bonbons-suedois.html" data-sv="Godis" data-fr="Bonbons suédois" data-en="Swedish candy">Bonbons suédois</a></li><li><a href="ahlgrens-bilar.html" data-sv="Ahlgrens Bilar" data-fr="Ahlgrens Bilar" data-en="Ahlgrens Bilar">Ahlgrens Bilar</a></li><li><a href="olw.html" data-sv="OLW dippmix" data-fr="Dips OLW" data-en="OLW dips">Dips OLW</a></li><li><a href="maison.html" data-sv="Inredning (Home)" data-fr="Maison (Home)" data-en="Home (homeware)">Maison (Home)</a></li><li><a href="recettes.html" data-sv="Recept" data-fr="Recettes" data-en="Recipes">Recettes</a></li></ul></div><div class="footer-col"><h4 data-sv="Kundservice" data-fr="Service" data-en="Service">Service</h4><ul><li><a href="livraison.html" data-sv="Leverans" data-fr="Livraison" data-en="Delivery">Livraison</a></li><li><a href="faq.html">FAQ</a></li><li><a href="contact.html" data-sv="Beställningsspårning" data-fr="Suivi commande" data-en="Order tracking">Suivi commande</a></li><li><a href="cgv.html" data-sv="Allmänna villkor" data-fr="CGV" data-en="T&Cs">CGV</a></li></ul></div><div class="footer-col"><h4>Contact</h4><ul><li><a href="mailto:${SD_WL.contact_email}">${SD_WL.contact_email}</a></li><li><a href="a-propos.html" data-sv="Vår historia" data-fr="Notre histoire" data-en="Our story">Notre histoire</a></li><li><a href="mentions-legales.html" data-sv="Rättsliga uppgifter" data-fr="Mentions légales" data-en="Legal notice">Mentions légales</a></li><li><a href="#" onclick="window.sdCookieSettings&&sdCookieSettings();return false;">Cookies</a></li></ul><p class="footer-origin">SE · FR</p></div></div><div class="footer-bottom"><p data-sv="${SD_WL.copyright.sv}" data-fr="${SD_WL.copyright.fr}" data-en="${SD_WL.copyright.en}">${SD_WL.copyright[LANG]}</p><div class="pay-badges"><span class="pay-badge">VISA</span><span class="pay-badge">MASTERCARD</span></div><p data-sv="${SD_WL.footer_tagline.sv}" data-fr="${SD_WL.footer_tagline.fr}" data-en="${SD_WL.footer_tagline.en}">${SD_WL.footer_tagline[LANG]}</p></div></div></footer>`;}

/* ── Bulle WhatsApp flottante (contact direct Victoria) ─────────────
   Présente sur toutes les pages (web + mobile). Animée « vivante »
   (halo qui pulse + léger rebond). Ouvre le WhatsApp du visiteur vers
   le +33 6 25 53 64 43 avec un message pré-rempli dans sa langue. */
function renderWhatsAppBubble(){
  if(document.getElementById('sd-wa-bubble'))return;
  var NUM='33625536443';
  var msg={fr:"Bonjour Victoria, j'ai une question 🙂",en:'Hello Victoria, I have a question 🙂',sv:'Hej Victoria, jag har en fråga 🙂'};
  var label={fr:'Discuter sur WhatsApp',en:'Chat on WhatsApp',sv:'Chatta på WhatsApp'};
  var lang=(typeof LANG!=='undefined'?LANG:'fr');
  if(!document.getElementById('sd-wa-style')){
    var st=document.createElement('style');st.id='sd-wa-style';
    st.textContent='#sd-wa-bubble{position:fixed;right:20px;bottom:22px;z-index:900;width:58px;height:58px;border-radius:50%;background:#25D366;display:flex;align-items:center;justify-content:center;box-shadow:0 6px 20px rgba(0,0,0,.22);animation:sd-wa-bob 2.6s ease-in-out infinite;transition:transform .15s;}'
      +'#sd-wa-bubble:hover{transform:scale(1.07);}'
      +'#sd-wa-bubble svg{width:32px;height:32px;fill:#fff;display:block;}'
      +'#sd-wa-bubble::after{content:"";position:absolute;inset:0;border-radius:50%;animation:sd-wa-pulse 2s ease-out infinite;}'
      +'@keyframes sd-wa-pulse{0%{box-shadow:0 0 0 0 rgba(37,211,102,.55)}70%{box-shadow:0 0 0 16px rgba(37,211,102,0)}100%{box-shadow:0 0 0 0 rgba(37,211,102,0)}}'
      +'@keyframes sd-wa-bob{0%,100%{transform:translateY(0)}50%{transform:translateY(-4px)}}'
      +'@media(max-width:767px){#sd-wa-bubble{bottom:90px;right:16px;width:52px;height:52px}#sd-wa-bubble svg{width:28px;height:28px}}'
      +'@media(prefers-reduced-motion:reduce){#sd-wa-bubble,#sd-wa-bubble::after{animation:none}}';
    document.head.appendChild(st);
  }
  var a=document.createElement('a');a.id='sd-wa-bubble';
  a.href='https://wa.me/'+NUM+'?text='+encodeURIComponent(msg[lang]||msg.fr);
  a.target='_blank';a.rel='noopener';
  a.setAttribute('aria-label',label[lang]||label.fr);a.title=label[lang]||label.fr;
