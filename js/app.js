...
function sdTrackAddToCart(p,qty){try{var v=(p&&p.price)||0;if(typeof gtag==='function')gtag('event','add_to_cart',{currency:'EUR',value:v,items:[{item_name:(p&&p.name&&p.name.fr)||'',quantity:qty||1,price:v}]});if(window.fbq)fbq('track','AddToCart',{value:v,currency:'EUR'});}catch(e){}}
function sdTrackPurchase(order){try{if(!order)return;var total=parseFloat(order.total)||0;var L=[];try{L=typeof order.lines==='string'?JSON.parse(order.lines):(order.lines||[]);}catch(e){}var items=L.map(function(l){return {item_name:l.name||'',quantity:l.qty||1,price:parseFloat(l.price)||0};});if(typeof gtag==='function')gtag('event','purchase',{transaction_id:String(order.order_number||order.id||''),value:total,currency:'EUR',items:items});if(window.fbq)fbq('track','Purchase',{value:total,currency:'EUR'});}catch(e){}}
