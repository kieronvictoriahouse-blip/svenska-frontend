...
function addToCart(id,qty=1,variant=null){if(id===undefined||id===null||String(id)==='NaN'||String(id)==='undefined')return;const p=PRODUCTS.find(x=>x.id===id||x.uuid===id||String(x.id)===String(id))||(window._SD_STATIC||[]).find(x=>x.id===id||String(x.id)===String(id));const cid=(p&&p.uuid)?p.uuid:id;const key=variant?`${cid}_${variant}`:`${cid}`;cart[key]=(cart[key]||0)+qty;saveCart();updateCartBadge();if(p){try{const cp=JSON.parse(localStorage.getItem('sd_cart_products')||'{}');cp[String(cid)]={name:p.name,photo:p.photo,price:p.price,weight:p.weight,variants:p.variants||[]};localStorage.setItem('sd_cart_products',JSON.stringify(cp));}catch(e){}const name=p.name[LANG];showToast(name.substring(0,28)+(name.length>28?'…':'')+' — '+UI[LANG].addedCart);}openCart();}
